# dense_baseline.py (Manual CPU Timing Version)
...
spark = SparkSession.builder.appName(f"DenseMul_Base_N{N}").getOrCreate()
...
def block_multiply(iter_data):
    ...
    for item in iter_data:
        ...
        results.append( ((block_i, block_j), np.dot(mat_A, mat_B)) ) # 核心计算
...
try:
    ...
    log_result_to_file(LOG_FILE, f"Type: Normal (Shuffle Join + CPU Time) | N: {N} | Block: {BLOCK_SIZE}")
    ...
    result = A_keyed.join(B_keyed).mapPartitions(block_multiply).reduceByKey(lambda m1, m2: m1 + m2)
    ...
    log_result_to_file(LOG_FILE, f"Status: Success | Blocks: {count}")
